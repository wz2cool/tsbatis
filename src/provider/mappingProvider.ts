...
import { DtoObjectHelper, EntityHelper } from "../helper";
import { DtoObject, Entity } from "../model";
import { ObjectUtils } from "ts-commons";

export class MappingProvider {
  public static toDtoObject<T extends DtoObject>(dtoObjectClass: T | { new (): T }, jsonObj: any): T {
  ...
  public static toDtoObjects<T extends DtoObject>(dtoObjectClass: T | { new (): T }, jsonObjs: any[]): T[] {
    return _.map(jsonObjs, jsonObj => {
      return MappingProvider.toDtoObject<T>(dtoObjectClass, jsonObj);
    });
  }

  public static toEntity<T extends Entity>(entity: T | { new (): T }, dbObj: any, underscoreToCamelCase = false): T {
  ...
  public static toEntities<T extends Entity>(entity: T | { new (): T }, dbObjs: any[], underscoreToCamelCase = false): T[] {
  ...
  private static toPropertyValue(dbValue: any, propertyType: string): any {
    ...
    if (ObjectUtils.isNullOrUndefined(dbValue)) {
      return null;
    }
