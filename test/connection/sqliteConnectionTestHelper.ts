...
export class SqliteConnectionTestHelper {
  public async testTransactionInsert(): Promise<void> {
    try {
      ...
      const sqliteConnection = await new SqliteConnection(filepath, true).init();
      ...
      const matchStudent = await sqliteConnection.selectEntities<Student>(Student, selectMatchStudentTemplate, [newStudent.name]);
