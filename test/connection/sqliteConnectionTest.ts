...
describe(".SqliteConnection", () => {
  describe("#getDataBaseType", () => {
    it("should return sqlite", async () => {
      const filepath = path.join(__dirname, "../../", "test", "northwind.db");
      const sqliteConnection = await new SqliteConnection(filepath).init();
      const result = sqliteConnection.getDataBaseType();
      expect(DatabaseType.SQLITE3).to.be.eq(result);
    });
  });

  describe("#getRowBoundsExpression", () => {
    it("should return `limit 20, 10` if rowbounds.offset is 20, rowbounds.limit 10", async () => {
      const filepath = path.join(__dirname, "../../", "test", "northwind.db");
      const sqliteConnection = await new SqliteConnection(filepath).init();
  ...
  describe("#selectEntities", async () => {
    ...
    it("should has error if invalid sql", done => {
      mysqlConnection
        .selectEntities<Customer>(Customer, "invalid sql", [])
        .then(data => {
          done("should have error if sql is invalid");
        })
        .catch(err => {
  ...
  describe("#transaction", () => {
    ...
    it("insertThenRollback", done => {
      testHelper
        .testTransactionInsertThenRollback()
        .then(() => {
          done();
        })
        .catch(err => {
